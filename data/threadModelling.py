...
def performThreadModelling(model_name, newThreadLabelObjects):
    ...
    results_dictionary = dict(zip(allThreadIDs, all_predicted_labels))
...
performThreadModelling("testM", labelled_threads_s)




# If we are concened about duplicates:
#indices = np.setdiff1d(np.arange(len(a)), np.unique(a, return_index=True)[1])

#print("labeled IDs: ", np.asarray(t['threadId'] for t in labelled_threads))
#print("labels: ", list(t['classId'] for t in labelled_threads))

#print(np.where(allThreadIDs == newLabelledIDs))



#print(indecesMatching)
